...
import { supabase } from '@/lib/supabaseClient'; // ✅ 공용 Supabase 클라이언트 사용
...
export function BlockchainExplorer() {
    ...
    try {
      ...
      if (electionsData) {
        for (const election of electionsData) {
          ...
          electionInfos.push({
            electionId: String(election.id), // ✅ 문자열로 통일
            exists: true,
            votingOpen,
            createdAt: Math.floor(
              new Date(election.created_at).getTime() / 1000
            ),
      ...
      if (votesData) {
        for (const vote of votesData) {
          ...
          records.push({
            electionId: String(vote.election_id), // ✅ 여기서도 문자열로 통일
  ...
  const filteredElections = elections.filter((election) => {
    const matchesSearch = String(election.electionId) // ✅ 안전하게 String으로 변환
      .toLowerCase()
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {elections.length > 0
                    ? `평균 ${(voteHistory.length / elections.length).toFixed(
                        1
                      )}표/선거`
                    : '-'}
                </p>
              ...
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {elections.filter((e) => !e.votingOpen).length > 0
                    ? '아카이브'
                    : '없음'}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {filteredElections.length > 0 && (
                  <span className="text-sm text-gray-600">
                    {filteredElections.length}개의 선거{' '}
                    {elections.length !== filteredElections.length &&
                      `(전체 ${elections.length}개 중)`}
                  </span>
                )}
              </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-gray-600">
                          {new Date(election.createdAt * 1000).toLocaleString(
                            'ko-KR'
                          )}
                        </span>
                      </div>
                        ...
                        <div className="bg-white p-2 rounded border">
                          ...
                          <span className="font-bold text-lg text-purple-600">
                            {election.candidateCount > 0
                              ? (
                                  election.totalVotes / election.candidateCount
                                ).toFixed(1)
                              : '0'}
                          </span>
              ...
              {voteHistory.length > 0 ? (
                <div className="space-y-2 max-h-96 overflow-y-auto pr-2">
                  {voteHistory
                    ...
                    .slice(0, 10)
                    .map((record, idx) => {
                      const relatedElection = elections.find(
                        (e) => e.electionId === record.electionId
                      );
                      return (
                        <div
                          key={idx}
                        ...
                        >
                          ...
                          <div className="bg-white p-2 rounded border">
                            ...
                            <button
                              onClick={() =>
                                copyToClipboard(record.voter, `voter-${idx}`)
                              }
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded border border-blue-200">
              <p className="text-sm text-blue-700">
                <span className="font-bold">ℹ️ 정보:</span> Supabase
                데이터베이스에서 실시간으로 선거 및 투표 데이터를 불러옵니다.
